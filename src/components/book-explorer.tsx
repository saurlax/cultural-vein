...
export function BookExplorer({
  ...
  const evidenceOverviewCards = useMemo(() => {
    return [
      {
        id: "people",
        label: "人物纪传",
        value: `${detail.realWorldSignals?.cbdbMatchedPeople ?? 0} 人`,
        note: `整理 ${detail.realWorldSignals?.cbdbFallbackPeople ?? 0} 人`,
      },
      {
        id: "venues",
        label: "场馆现场",
        value: `${detail.realWorldSignals?.venueSamples?.length ?? 0} 组`,
        note: `${detail.realWorldSignals?.eventSamples?.length ?? 0} 条活动`,
      },
      {
        id: "borrow",
        label: "公共流通",
        value: `${detail.realWorldSignals?.borrowLibraries?.length ?? 0} 馆`,
        note: `${detail.realWorldSignals?.borrowSamples?.length ?? 0} 条借阅`,
      },
      {
        id: "institutions",
        label: "机构馆藏",
        value: `${detail.realWorldSignals?.institutionSamples?.length ?? 0} 条`,
        note: `${new Set((detail.realWorldSignals?.institutionSamples ?? []).map((item) => item.institution)).size} 家机构`,
      },
    ];
  }, [
    detail.realWorldSignals?.borrowLibraries,
    detail.realWorldSignals?.borrowSamples,
    detail.realWorldSignals?.cbdbFallbackPeople,
    detail.realWorldSignals?.cbdbMatchedPeople,
    detail.realWorldSignals?.eventSamples,
    detail.realWorldSignals?.institutionSamples,
    detail.realWorldSignals?.venueSamples,
  ]);
  const institutionEvidenceGroups = useMemo(() => {
    ...
    for (const record of institutionRecords) {
      const existing = groups.get(record.institution);

      if (existing) {
        existing.count += 1;
        if (!existing.latestYear && record.year) {
          existing.latestYear = record.year;
        }
        continue;
      }

      groups.set(record.institution, {
        institution: record.institution,
        count: 1,
        latestYear: record.year ?? null,
      });
    }
        ...
        <section className="rounded-[24px] border border-amber-300/15 bg-[linear-gradient(180deg,rgba(183,129,39,0.14),rgba(77,49,15,0.16))] px-4 py-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {sourceBadges.map((item) => (
              <button
                ...
                type="button"
                onClick={() => handleOpenSourceEvidence("institution-samples")}
                className="rounded-full border border-amber-300/18 bg-[rgba(255,244,214,0.08)] px-3 py-1 text-xs text-amber-100 transition hover:bg-[rgba(255,244,214,0.12)]"
              >
                {item}
              </button>
            ))}
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <button
              ...
              className="rounded-2xl border border-amber-300/18 bg-[rgba(255,244,214,0.08)] px-3 py-3 text-left transition hover:bg-[rgba(255,244,214,0.12)]"
            >
              <div className="text-xs tracking-[0.2em] text-amber-100/70">
                人物纪传
              </div>
              <div className="mt-2 text-sm text-[#fbf3da]">
          ...
          <div className="mt-4 grid gap-3 xl:grid-cols-3">
            <div className="rounded-2xl border border-amber-300/18 bg-[rgba(255,244,214,0.08)] px-4 py-4">
              <div className="flex items-center justify-between gap-3">
              ...
              <div className="mt-3 space-y-2">
                {venuePreview.length ? (
                  venuePreview.map((venue) => (
                    ...
                    >
                      <div className="font-medium text-[#fbf3da]">{venue.name}</div>
                      <div className="mt-1 text-xs text-[#cdb98d]">
                  ...
                  <div className="rounded-2xl border border-dashed border-[#d8b56f]/24 bg-[rgba(255,244,214,0.04)] px-3 py-4">
                    <div className="text-sm text-[#eadfbc]">这一层尚未落到具体场馆，时间与传播两条线仍在为现实去处保留回声。</div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => setManualTab("timeline")}
                ...
                {eventPreview.length ? (
                  eventPreview.map((event) => (
                    <button
                      ...
                      className="w-full rounded-2xl border border-[#d8b56f]/18 bg-[rgba(255,244,214,0.08)] px-3 py-3 text-left text-sm transition hover:bg-[rgba(255,244,214,0.12)]"
                    >
                      <div className="font-medium text-[#fbf3da]">{event.title}</div>
                      <div className="mt-1 text-xs text-[#cdb98d]">
                  ...
                  <div className="rounded-2xl border border-dashed border-[#d8b56f]/24 bg-[rgba(255,244,214,0.04)] px-3 py-4">
                    <div className="text-sm text-[#eadfbc]">这一层尚未落到具体活动事件，传播与人物两条支流仍在托住扩散路径。</div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => setManualTab("spread")}
                ...
                {activeSourceEvidence ? (
                  <div className="rounded-2xl border border-amber-300/12 bg-[rgba(255,248,220,0.06)] px-4 py-4">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                      {evidenceOverviewCards.map((card) => (
                        <div
                          key={`evidence-overview-${card.id}`}
                          className="rounded-2xl border border-[#d8b56f]/18 bg-[rgba(255,244,214,0.08)] px-3 py-3"
                        >
                          <div className="text-[10px] tracking-[0.18em] text-[#d8c9a3]">
                            {card.label}
                          </div>
                          ...
                          <div className="mt-1 text-[11px] leading-5 text-[#cdb98d]">
                            {card.note}
                          </div>
                        </div>
                      ))}
                    </div>
                    {institutionEvidenceGroups.length ? (
                      <div className="mt-3 rounded-2xl border border-[#d8b56f]/18 bg-[rgba(255,244,214,0.08)] px-4 py-4">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-sm text-[#eadfbc]">
                              当前这本书已经落到 {institutionEvidenceGroups.length} 家真实机构的馆藏或专题线索里。
                            </div>
                          </div>
                          <div className="rounded-full border border-amber-300/18 bg-amber-300/10 px-3 py-1 text-[10px] text-amber-100">
                            机构归流
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {institutionEvidenceGroups.slice(0, 6).map((group) => (
                            <button
                              key={`institution-group-${group.institution}`}
                              type="button"
                              onClick={() => {
                                const targetRecord = institutionRecords.find(
                                  (item) => item.institution === group.institution,
                                );
                                if (targetRecord) {
                                  handleSelectInstitutionRecord(targetRecord);
                                }
                              }}
                              className="rounded-full border border-[#d8b56f]/18 bg-[rgba(255,248,220,0.06)] px-3 py-1.5 text-left text-xs text-[#fbf3da] transition hover:bg-[rgba(255,248,220,0.1)]"
                            >
                              {group.institution} · {group.count} 条
                              {group.latestYear ? ` · ${group.latestYear}` : ""}
                            </button>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="mt-3 rounded-2xl border border-[#d8b56f]/18 bg-[rgba(255,248,220,0.05)] px-4 py-4">
                      <div className="flex items-center justify-between gap-3">
                        <div className="text-xs tracking-[0.2em] text-[#d8c9a3]">
                          示范样本
                        </div>
                        <div className="rounded-full border border-[#d8b56f]/18 bg-[rgba(255,244,214,0.08)] px-3 py-1 text-[10px] text-[#eadfbc]">
                          {activeSourceEvidence.samples.length} 条
                        </div>
                      </div>
                      <div className="mt-3 grid gap-2">
                      ...
                      {visibleSpread.map((item, index) => {
                        ...
                        return (
                          <div key={item.id} className="flex min-w-max items-center gap-2">
                            ...
                            >
                              <div className="text-xs tracking-[0.18em] text-[#d8c9a3]">
                                航段 {index + 1}
                              </div>
                              <div className="mt-2 text-sm font-medium text-[#fbf3da]">
                                {fromPlace?.name ?? "未知"} → {toPlace?.name ?? "未知"}
                              </div>
                            <div className="mt-2 flex items-center gap-2 text-xs text-[#cdb98d]">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {activeSpread ? (
                        <div className="mt-4 space-y-3">
                          ...
                          >
                            <div className="flex items-center justify-between gap-3">
                              <span className="text-sm font-medium text-[#fbf3da]">
                                传播时间
                              </span>
                              <span className="text-xs text-[#cdb98d]">
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            ...
                            >
                              ...
                              <p className="mt-2 text-sm leading-6 text-[#eadfbc]">
                                {activeSpreadPlaces?.to?.note ?? "这座终点城市承接当前航段，可继续回到时间线或人物关系追看扩散回声。"}
                              </p>
                              <div className="mt-3 text-xs text-amber-100/80">
                                转看人物关系，继续讲清这段扩散在人物层如何接力。
                              </div>
                            </button>
                          </div>
                        </div>
